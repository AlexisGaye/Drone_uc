...
void PWM_Init() {
  ...
  // PC6789
  GPIOC->MODER &=
      ~((3 << 12) | (3 << 14) | (3 << 16) | (3 << 18));          // return to 00
  GPIOC->MODER |= (1 << 13) | (1 << 15) | (1 << 17) | (1 << 19); // PC6/7/8/9
                                                                 // :AF
  GPIOC->OTYPER &= ~((1 << 6) | (1 << 7) | (1 << 8) | (1 << 9)); // PP
  GPIOC->PUPDR |= (1 << 12) | (1 << 14) | (1 << 16) | (1 << 18); // up
  GPIOC->OSPEEDR |=
      ((3 << 12) | (3 << 14) | (3 << 16) | (3 << 18)); // very High speed
  ...
  TIM3->CCMR1 |= (1 << 3) | (1 << 11); // CH1 CH2 Output compare 1 preload
                                       // enable
  TIM3->CCMR2 |= (6 << 4) | (6 << 12); // CH3 CH4 PWM mode 1
  TIM3->CCMR2 |= (1 << 3) | (1 << 11); // CH3 CH4 Output compare 1 preload
                                       // enable

  TIM3->CCER |=
      1 | (1 << 4) | (1 << 8) | (1 << 12); // Compare 1/2/3/4 output enable
...
void ChangeAccelrator(int16_t motor[]) {
  motor[0] =
      (int16_t)(((float)(receiver.ch[3] - 0x03E8)) / (0x07C4 - 0x03E8) * 1000 +
                1000);
  TIM3->CCR1 = motor[0];
  TIM3->CCR2 = motor[0];
  TIM3->CCR3 = motor[0];
